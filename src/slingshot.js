// slingshot.js — Unified pointer-event pull interaction
// Ported from indrolend/basic-browser-spa (js/spa/slingshotGesture.js).
// No SPA dependencies — pure module, safe to use anywhere.
//
// API: initSlingshot(element, callbacks) → { destroy }
//
// callbacks:
//   onArm()          — pointer went down; gesture is armed
//   onLock(info)     — drag threshold crossed; direction committed
//                      return false to reject the lock and abort
//   onPull(info)     — active pull update; info = { pullVector, pullNormalized }
//   onRelease(info)  — pointer lifted after a locked drag; info = { direction, pullVector, pullNormalized }
//   onCancel()       — pointer was cancelled (e.g. interrupted by phone call)
//   onTap()          — pointer lifted without reaching drag threshold (treat as tap)
...
/**
 * Attach a slingshot gesture to `element`.
 *
  ...
  function finalize(e, isCancelled) {
    ...
    if (isCancelled || !wasLocked) {
      if (isCancelled) {
        ...
        // Pointer released without reaching the drag threshold — treat as tap.
    ...
    if (typeof onRelease === 'function') {
      onRelease({
        ...
        pullVector: savedPullVector,
        pullNormalized: savedPullNormalized
      });
    }
  }
