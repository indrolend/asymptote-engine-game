// particle-transition.js — Canvas particle transition engine
// Ported from indrolend/basic-browser-spa (js/spa/particleTransitionEngine.js).
// No SPA dependencies — pure canvas module, safe to use anywhere.
//
// API:
//   transition(fromCanvas, toCanvas, options, onComplete)
//     Animate an explode → reform between two pre-rasterised canvases.
//     options: { ctx, fromRegion?, toRegion?, timingProfile? }
//
//   transitionFromPull(pulledParticles, toRegion, ctx, options, onComplete)
//     Continue a slingshot pull directly into a reform animation, with no
//     explode phase — particles travel from their drag positions to the target.

const PARTICLE_SIZE = 4;

// ─── Shared helpers ───────────────────────────────────────────────────────────

/** Scratch canvas reused across sampleParticles calls (sequential, not concurrent). */
 ...
 *   options.ctx         — CanvasRenderingContext2D to draw on
 *   options.fromRegion  — { canvas, width, height } (defaults to fromCanvas bounds)
 *   options.toRegion    — { canvas, width, height } (defaults to toCanvas bounds)
 *   options.timingProfile — 'chained' for shorter durations when chaining transitions
 ...
 * Particles start from their pulled preview positions and converge onto the
 * target surface — no explode phase, truthful continuation of the drag.
 *
 ...
 * @param {Object} options
 *   options.fromParticlesBase — base (un-pulled) particle positions for the
 *                               elastic snap-back phase (optional)
 * @param {Function} onComplete
 */
export function transitionFromPull(pulledParticles, toRegion, ctx, options, onComplete) {
  const width  = ctx.canvas.width;
  const height = ctx.canvas.height;
  const SNAP_DURATION   =  80; // ms — elastic snap-back: pulled → rest positions
  const REFORM_DURATION = 270; // ms — reform: rest positions → target shape
  ...
  function animate(ts) {
    ...
    if (hasSnap && elapsed < SNAP_DURATION) {
      // Phase 1: snap-back — pulled → from-surface rest positions (ease-out quad)
